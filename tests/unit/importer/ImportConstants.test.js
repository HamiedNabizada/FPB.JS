...
describe('ImportConstants', () => {
  ...
  describe('IMPORT_EVENTS', () => {
    ...
    it('defines all 8 event types', () => {
      expect(Object.keys(IMPORT_EVENTS)).toHaveLength(8);
    });

    it('defines IMPORT_DONE', () => {
      expect(IMPORT_EVENTS.IMPORT_DONE).toBe('import.done');
    });

    it('defines IMPORT_REPORT', () => {
      expect(IMPORT_EVENTS.IMPORT_REPORT).toBe('import.report');
    });
